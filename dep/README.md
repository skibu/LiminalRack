...
`git submodule add <git_url> dep/<directory>` to update things. For example, 
to add the cpptrace library should do:
```
LiminalRack$ git submodule add -f https://github.com/jeremy-rifkin/cpptrace.git dep/cpptrace
```

```
These commands were for boost but no longer needed
