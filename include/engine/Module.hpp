 ...
 * Note: since Module is a base class for all the custom modules out there, this
...
struct Module {
    ...
    */
    ...
    It is recommended to call getParam(), getInput(), etc. instead of accessing
    these directly.
    */
    ...
    Initialized using configParam(), configInput(), configOutput(), and
    configLight(). LightInfos are initialized to null unless configLight() is
    called.

    It is recommended to call getParamQuantity(), getInputInfo(), etc. instead
    of accessing these directly.
    */
    ...
    struct Expander {
        ...
        If you intend to receive messages from an expander, allocate both
        message buffers with identical blocks of memory (arrays, structs, etc).
        ...
        its message buffer. Once the module is checked, you can reinterpret_cast
        its producerMessage at no performance cost.

        Producer messages are intended to be write-only.
        Consumer messages are intended to be read-only.

        Once you write a message, set messageFlipRequested to true to request
        that the messages are flipped at the end of the timestep. This means
        that message-passing has 1-sample latency.

        You may choose for your Module to instead write to its own message
        buffer for consumption by other modules, i.e. the expander "pulls"
        rather than this module "pushing". As long as this convention is
        followed by the other module, this is fine.
        */
    ...
    Module();
    /** Use config() instead. */
    DEPRECATED Module(int numParams, int numInputs, int numOutputs,
                      int numLights = 0)
        : Module() {
    ...
    */
    void config(int numParams, int numInputs, int numOutputs,
                int numLights = 0);
    ...
    */
    ...
    TParamQuantity* configParam(int paramId, float minValue, float maxValue,
                                float defaultValue, std::string name = "",
                                std::string unit = "", float displayBase = 0.f,
                                float displayMultiplier = 1.f,
                                float displayOffset = 0.f) {
        assert(paramId < (int)params.size() &&
               paramId < (int)paramQuantities.size());
        if (paramQuantities[paramId]) delete paramQuantities[paramId];
    ...
    */
    ...
    TSwitchQuantity* configSwitch(int paramId, float minValue, float maxValue,
        ...
        TSwitchQuantity* sq = configParam<TSwitchQuantity>(
            paramId, minValue, maxValue, defaultValue, name);
    ...
    */
    ...
    TSwitchQuantity* configButton(int paramId, std::string name = "") {
        TSwitchQuantity* sq =
            configParam<TSwitchQuantity>(paramId, 0.f, 1.f, 0.f, name);
        sq->ParamQuantity::snapEnabled = true;
        sq->ParamQuantity::smoothEnabled = false;
    ...
    /** Helper for creating a PortInfo for an input port and setting its
    properties. See PortInfo for documentation of arguments. Should only be
    called from a Module subclass's constructor.
    */
    ...
    TPortInfo* configInput(int portId, std::string name = "") {
        ...
        if (inputInfos[portId]) delete inputInfos[portId];
    ...
    /** Helper for creating a PortInfo for an output port and setting its
    properties. See PortInfo for documentation of arguments. Should only be
    called from a Module subclass's constructor.
    */
    template <class TPortInfo = PortInfo>
    TPortInfo* configOutput(int portId, std::string name = "") {
        assert(portId < (int)outputs.size() &&
               portId < (int)outputInfos.size());
        if (outputInfos[portId]) delete outputInfos[portId];
    ...
    */
    ...
    TLightInfo* configLight(int lightId, std::string name = "") {
        assert(lightId < (int)lights.size() &&
               lightId < (int)lightInfos.size());
        if (lightInfos[lightId]) delete lightInfos[lightId];
    ...
    /** Adds a direct route from an input to an output when the module is
    bypassed. Should only be called from a Module subclass's constructor.
    */
    void configBypass(int inputId, int outputId) {
        ...
        for (BypassRoute& br : bypassRoutes) {
            // Prevent unused variable warning for compilers that ignore
            // assert()
            (void)br;
    ...
    Do not call this method in process() since filesystem operations block the
    audio thread.
    ...
    Patch storage files of deleted modules are garbage collected when user saves
    the patch. To allow the Undo feature to restore patch storage if the module
    is accidentally deleted, it is recommended to not delete patch storage in
    onRemove().
    */
    ...
    /** Returns the left Expander for `side = 0` and the right Expander for
     * `side = 1`. */
    ...
    Override this method to read Inputs and Params and to write Outputs and
    Lights.
    */
    ...
    If you do override it, avoid reading param values, since the state of the
    module should have no effect on routing.
    */
    virtual void processBypass(const ProcessArgs& args);

    /** Usually you should override dataToJson() instead.
    There are very few reasons you should override this (perhaps to lock a mutex
    while serialization is occurring).
    */
    virtual json_t* toJson();
    /** This is virtual only for the purpose of unserializing legacy data when
    you could set properties of the `.modules[]` object itself. Normally you
    should override dataFromJson(). Remember to call `Module::fromJson(rootJ)`
    ...
    */
    ...
    /** Override to store extra internal data in the "data" property of the
     * module's JSON object. */
    virtual json_t* dataToJson() {
        return NULL;
    }
    /** Override to load internal data from the "data" property of the module's
    JSON object. Not called if "data" property is not present.
    */
    ...
    This event is not called for output ports if a stackable cable was
    added/removed and did not change the port's connected state.
    */
    ...
    /** Called when the Engine sample rate changes, and when the Module is added
     * to the Engine.
     */
    ...
    /** Called after an expander is added, removed, or changed on either the
     * left or right side of the Module.
     */
    virtual void onExpanderChange(const ExpanderChangeEvent& e) {}

    struct ResetEvent {};
    /** Called when the user resets (initializes) the module.
    The default implementation resets all parameters to their default value, so
    you must call `Module::onReset(e)` in your overridden method if you want to
    keep this behavior.
    */
    ...
    The default implementation randomizes all parameters by default, so you must
    call `Module::onRandomize(e)` in your overridden method if you want to keep
    this behavior.
    */
    ...
    If your module uses patch asset storage, make sure all files are saved in
    this event.
    */
