 ...
 * Caches the unchanging value so don't have to query everytime.
*/
int getLogicalCoreCount();

/** Returns the number of physical CPU cores on the system. This can be
 * different from logical core count due to "hyperthreading" where can have 2
 * logical cores per physical core. Caches the unchanging value so don't have to
 * query everytime.
 */
int getPhysicalCoreCount();

/** Returns the CPU usage percentage. Uses retrieveSystemCpuPercentage() and
 * caching to avoid querying too often.
 */
float getSystemCpuPercentage();

/** Actually determines the CPU usage percentage. Does this by using unix
 * command line to sum the CPU usage for all processes, and then dividing by the
 * number of physical cores. This way don't get a value over 100% due to
 * mutliple cores being used simultaneously. Only works for Macs and Linux
 * currently. Does not work for Windows.
 * 
 * Should not be called directly, instead use getSystemCpuPercentage() which
 * caches the result for a second to avoid querying too often.
 */
...
/** Returns the user time used by the current thread in seconds.
*/
double getThreadTime();

/** Puts the current thread to sleep for a specified duration (in seconds). */
...
/** Executes command and returns the resulting output. If the command fails,
...
/** Sets the CPU's floating point unit control flags.
MXCSR register on x64, and the FPCR register on ARM64.
*/
