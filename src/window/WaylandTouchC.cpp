...
static void wlTouchDownClbk(void* data, struct wl_touch* wl_touch,
    ...
    TRACE("Wayland touch down event: id=%d, x=%f, y=%f", id,
          wl_fixed_to_double(x), wl_fixed_to_double(y));

    rack::window::WaylandTouch::downEventCallback(
        serial, time, id, wl_fixed_to_double(x), wl_fixed_to_double(y));
}

/** Callback that is called as soon as a touch up event occurs */
static void wlTouchUpClbk(void* data, struct wl_touch* wl_touch,
...
static void wlTouchMotionClbk(void* data, struct wl_touch* wl_touch,
                              ...
                              wl_fixed_t y) {
    TRACE("Wayland touch motion event: id=%d, x=%f, y=%f", id,
          wl_fixed_to_double(x), wl_fixed_to_double(y));
...
/** Called when get an event that specifies the shape & orientation of the touch
 * contact. Unfortunately don't seem to ever receive this kind of event. If did
 * could possibly use it to implement something fancy, but doesn't seem that
 * important.
 */
static void wlTouchOrientationClbk(void* data, struct wl_touch* wl_touch,
    ...
    rack::window::WaylandTouch::orientationEventCallback(
...
static void registryGlobalClbk(void* data, struct wl_registry* wl_registry,
    ...
    if (strcmp(interface, wl_seat_interface.name) == 0) {
        ...
        TRACE("Adding Wayland registry global: interface %s (version %d)",
              interface, version);
...
static void registryGlobalRemoveClbk(void* data,
                                     struct wl_registry* wl_registry,
                                     uint32_t name) {
